function [outHR, cntH] = heartpulserate(currHR_val)
...
for i = 1:length(currHR_val)
    ...
    if hr_state == 0
        ...
        if hr_peak-hr_baseline>50 && currHR_val(i) > hr_baseline + hr_threshold
            hr_state = 2;
            hr_mstick = round(toc * 10000);
            hr_pulsecnt = 0;
        end
    elseif hr_state == 1
        if currHR_val(i) > hr_baseline + hr_threshold
            hr_state = 2;   
            hr_period(hr_pulsecnt) = round(toc * 10000)-hr_mstick;
            hr_mstick = round(toc * 10000);
        else 
            if (round(toc * 10000) - hr_mstick > 2000) 
                hr_state = 0;
            end
        end
    elseif hr_state == 2
        if currHR_val(i) < hr_peak - hr_threshold
            if flg_esta == 0
                hr_state = 1;
                if hr_pulsecnt>0
                    hr_period(hr_pulsecnt) = hr_period(hr_pulsecnt) + (round(toc * 10000)-hr_mstick);
                    if (hr_pulsecnt == 3) 
                ...
                hr_peak = max(rawHR_stack(i-hr_datacnt:i));
                hr_threshold = (hr_peak-hr_baseline)/1.5;
                hr_period(1) = hr_period(1) + round(toc * 10000)-hr_mstick;
                hr_mstick = round(toc * 10000);
                hr_std = std(hr_period);
                hr_mean = mean(hr_period);
                if hr_mean > 200 && hr_mean < 3000 && hr_std < 500
            ...
            if round(toc * 10000) - hr_mstick > 1000
                hr_state = 0;
                flg_esta = 0;
            end
        end
